...
public class ModCrafting {
	...
	public static class Ingredients {
		...
		private static final DeferredRegister<IngredientType<?>> INGREDIENT_TYPES = DeferredRegister.create(NeoForgeRegistries.INGREDIENT_TYPES, OldGuns.MODID);

		//public static final DeferredHolder<IngredientType<?>, ? extends IngredientType<?>> NEVER =
		// INGREDIENT_TYPES.register(new ResourceLocation(OldGuns.MODID, "never"), IngredientNever::getType);


				//INGREDIENT_TYPES.register(new ResourceLocation(OldGuns.MODID, "never"), IngredientNever.INSTANCE.getType());
	...
	public static class Recipes {
		...
		public static final DeferredHolder<RecipeSerializer<?>, ? extends ShapelessVanillaMuzzleloaderReloadRecipe.Serializer>
				FIREARM_MUZZLELOADER_RELOAD_SHAPELESS = RECIPE_SERIALIZERS
				.register("firearm_muzzleloader_reload_shapeless", ShapelessVanillaMuzzleloaderReloadRecipe.Serializer::new);
	...
	public static <C extends Container, T extends Recipe<C>> Map<ResourceLocation, Recipe<C>> getRecipes(Level world, RecipeType<T> type) {
		return  (Map<ResourceLocation, Recipe<C>>) world.getRecipeManager().getRecipeIds()
				.collect(Collectors.toMap(v -> v, v -> world.getRecipeManager().byKey(v).orElseThrow(IllegalArgumentException::new).value()));
	}
}
