...
public class LowGradeBlackPowderBlock extends FallingBlock {
	...
	public void randomTick(BlockState state, ServerLevel level, BlockPos blockpos, RandomSource rand) {
		...
		if (canGetWet) {
			...
			if (allowWet) {
				level.setBlockAndUpdate(blockpos, ModBlocks.WET_LOW_GRADE_BLACK_POWDER_BLOCK.get().defaultBlockState());
			}
		}
	}
	...
	public void onLand(Level level, BlockPos blockpos, BlockState stateA, BlockState stateB, FallingBlockEntity p_52072_) {
		if (canGetWet(level, blockpos, stateB)) {
			level.setBlock(blockpos, ModBlocks.WET_LOW_GRADE_BLACK_POWDER_BLOCK.get().defaultBlockState(), 3);
		}
	}
	...
	public BlockState getStateForPlacement(BlockPlaceContext stateContext) {
		...
		return canGetWet(blockgetter, blockpos, blockstate) ? ModBlocks.WET_LOW_GRADE_BLACK_POWDER_BLOCK.get().defaultBlockState() : super.getStateForPlacement(stateContext);
	}
	...
	public BlockState updateShape(BlockState stateA, Direction facing, BlockState stateB, LevelAccessor level, BlockPos blockposA, BlockPos blockposB) {
		return touchesLiquid(level, blockposA) ? ModBlocks.WET_LOW_GRADE_BLACK_POWDER_BLOCK.get().defaultBlockState() : super.updateShape(stateA, facing, stateB, level, blockposA, blockposB);
	}
