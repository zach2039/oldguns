...
public class WetLowGradeBlackPowderBlock extends Block {
	...
	public WetLowGradeBlackPowderBlock() {
	...
	public void randomTick(BlockState state, ServerLevel level, BlockPos blockpos, RandomSource rand) {
		...
		if (canDry) {
			...
			if (allowDrying) {
				level.setBlockAndUpdate(blockpos, ModBlocks.LOW_GRADE_BLACK_POWDER_BLOCK.get().defaultBlockState());
			}
		}
	}
